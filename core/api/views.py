...
class SubStageAPI(APIView):
    ...
    def get(self, *args, **kwargs):
        ...
        if stage_id:
            sub_stage = SubStage.objects.prefetch_related.filter(parent_stage=stage_id)
...
class QuestionAPI(APIView):
    ...
    def get(self, *atgs, **kwargs):
        ...
        if sub_stage_id:
            question = Question.objects.filter(sub_stage=sub_stage_id)
...
class testAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for stage_data in data:
            ...
            form_stage = FormStage(title=stage_data['title'], form_result=form_results[0])
            form_stages.append(form_stage)

            for sub_stage_data in stage_data['sub_stages']:
                if validate_json(sub_stage_data, 'title'):
                    return JsonResponse({"message": validate_json(sub_stage_data, 'title')})
                
                form_sub_stage = FormSubStage(title=sub_stage_data['title'], form=form_stage)
                ...
                for question_data in sub_stage_data['questions']:
                    if validate_json(question_data, 'question'):
                        return JsonResponse({"message": validate_json(question_data, 'question')})

                    form_question = FormQuestion(question=question_data['question'], form_sub_stage=form_sub_stage)
                    ...
                    for answer in question_data['answers']:
                        ...
                        form_answer = FormAnswer(answer=answer['answer'], form_question=form_question)
